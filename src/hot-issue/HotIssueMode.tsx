...
import { HotIssueForm } from './HotIssueForm'
import { HotIssuePreview } from './HotIssuePreview'
import { useHotIssueDraft } from './useHotIssueDraft'

type Tab = 'edit' | 'preview'

export function HotIssueMode() {
  const draft = useHotIssueDraft()
    ...
    try {
      ...
      setLoadStatus(
        result === 'loaded'
          ? '저장소의 핫이슈 데이터를 불러왔어요.'
          : result === 'empty'
            ? '저장소에 준비된 데이터(hot-issue/current.json)가 없어요.'
            : null,
      )
    } catch (err) {
    ...
    <>
      ...
      <div className="mx-auto max-w-xl px-4 pt-4">
        <button
          ...
          className="w-full rounded-lg border border-sky-300 bg-white py-2.5 text-sm font-medium text-sky-800 active:bg-sky-50"
        >
          저장소에서 불러오기 (hot-issue/current.json)
        </button>
        {loadStatus && <p className="mt-2 text-xs text-gray-500">{loadStatus}</p>}

        {data.status === 'ready' && data.generatedAt && (
          <p className="mt-2 text-xs text-gray-400">
            {data.meta.topic} · 데이터 기준{' '}
      ...
      {tab === 'edit' ? (
        <main className="mx-auto max-w-xl px-4 py-5">
          <HotIssueForm draft={draft} />
        </main>
      ) : (
        <main className="py-5">
          <HotIssuePreview draft={draft} />
        </main>
      )}
    </>
  )
